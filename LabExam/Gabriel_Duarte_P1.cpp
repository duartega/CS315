...
private:
  struct BinaryNode
  {
    ...
    BinaryNode(const Comparable &theElement, BinaryNode *lt, BinaryNode *rt, int sz, int ht, bool dt, int ct, int kc)
        : element(theElement), left(lt), right(rt), size(sz), height(ht), deleted(dt), count(ct), keyCount(kc) {}
  };
  ...
  void insert(const Comparable &x, BinaryNode *&t)
  {
    if (t == NULL)
    {
      t = new BinaryNode(x, NULL, NULL, 1, 0, false, 1, 1);
    }
    else if (x < t->element)
    {
      insert(x, t->left);
  ...
  BinaryNode *clone(BinaryNode *t) const
  {
    if (t == NULL)
      ...
      return new BinaryNode(t->element, clone(t->left), clone(t->right), t->size, t->height, t->deleted, t->count, t->keyCount);
  }
